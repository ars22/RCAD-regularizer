...
class AdvEntropy(tf.keras.Model):
  ...
  @tf.function
  def adv_step(self, X, Y, step_size):
    if step_size > 0.:
      with tf.GradientTape(watch_accessed_variables=False) as tape:
        tape.watch(X)
        Y_logits = self.model(X, training=True)
        kl_loss = KLUnif(Y_hat, from_logits=True)
      grads = tape.gradient(kl_loss, X)
      grads_norm = tf.norm(tf.reshape(grads, (tf.shape(X)[0], -1)), axis=1)
      grads = self.args.step_size * grads / grads_norm[:, None, None, None]      
  ...
  def train_step(self, data):
    ...
    return {"total_loss": self.total_loss_tracker.result(), "ce_loss": self.ce_loss_tracker.result(), 
    "adv_loss": self.adv_loss_tracker.result(), "ent": self.ent_tracker.result(), "accuracy": self.acc_tracker.result()}
  ...
  def metrics(self):
    ...
    return [self.acc_tracker, self.total_loss_tracker, self.ce_loss_tracker, self.adv_loss_tracker, self.ent_tracker]
...
def run():
  ...
  trainer = AdvEntropy(model=model, args=args)
